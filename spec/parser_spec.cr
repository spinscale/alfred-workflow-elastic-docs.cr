...
describe Parser do
  ...
  it "should parse" do
    parser.parse("foo").should eq({ "query" => "foo", "product" => nil, "version" => "7.14" })
  end

  it "should parse products" do
    parser.parse("a foo bar").should eq({ "query" => "foo bar", "product" => "APM", "version" => "7.14" })
    parser.parse("e foo").should eq({ "query" => "foo", "product" => "Elasticsearch", "version" => "7.14" })
  end
